...
class Player:
    ...
    def movement(self, user_input):
        ...
        if user_input[pygame.K_DOWN] or user_input[pygame.K_s]:
            if self.velocity.length() > 0 and self.brake_hold_time == False:
                ...
                if ((brake_dir.x > 0 > self.velocity.x) or
                        (brake_dir.x < 0 < self.velocity.x) or
                (brake_dir.y > 0 > self.velocity.y) or
                        (brake_dir.y < 0 < self.velocity.y)):
                    self.velocity.x = 0
                    self.velocity.y = 0
        ...
        no_movement_keys = not (user_input[pygame.K_UP] or
                                user_input[pygame.K_w] or
                            user_input[pygame.K_DOWN] or
                                user_input[pygame.K_s])
        ...
        if no_movement_keys or self.brake_hold_time:
            ...
            if self.velocity.length() == 0:
                self.velocity.y = min(self.velocity.y + self.drift_speed,
        ...
        # Steering
        turn_speed = self.rotation_speed * (0.2 + 0.8 *
    ...
    def check_road_collision(self, background_):
        ...
        for point_x, point_y in check_points:
            ...
            if (0 <= tile_y < len(background_.road_map) and
                0 <= tile_x < len(background_.road_map[0])):
                if background_.road_map[tile_y][tile_x] == 0:
...
class AiCar:
    def __init__(self, lane):
        ...
        self.image = (pygame.image.load(f"ai_{self.colour_name}_car.png")
                      .convert_alpha())
        ...
        self.lane = lane
        self.boost_multiplier = 2.0  # How much they accelerate
...
class Background:
    ...
    def generate_road_map(self):
        ...
        background_ = [[0 for _ in range(columns)] for _ in range(rows)]
    ...
    def update(self, keys_):
        # Handle acceleration/deceleration
        if keys_[pygame.K_UP] or keys_[pygame.K_w]:
            self.speed = min(self.speed + self.acceleration, self.max_speed)
        elif keys_[pygame.K_DOWN] or keys_[pygame.K_s]:
...
# in game score
def draw_score(screen_):
    score_text = SMALL_FONT.render(f"Score: {score}", True, WHITE)
    pygame.draw.rect(screen_, GRASS_COLOUR, (10, 10,
                                             score_text.get_width()
                                            + 20,
                                            score_text.get_height() + 10))
    screen_.blit(score_text, (20, 15))
...
def show_end_screen():
    ...
    high_score_text = SMALL_FONT.render(f"High Score: {high_score}",
                                        True, WHITE)
    screen.blit(high_score_text, (WIDTH//2 - high_score_text.get_width()
                                  //2, HEIGHT//2))

    # score
    score_text = SMALL_FONT.render(f"Your Score: {score}", True,
                                   WHITE)
...
def update_high_score(score_, high_score_):
    if int(score_) > int(high_score_):
        return score_
    else:
        return high_score_

# saving high scores
def save_high_score(high_score_):
    high_score_file = open("Hi_score.txt", 'w')
    high_score_file.write(str(high_score_))
    ...
    for i, item in enumerate(menu_items):
        ...
        text = font.render(item["text"], True, MENU_HOVER_COLOR if 
                         (WIDTH//2 - 100 <= mouse_pos[0] <= WIDTH//2 + 100 and
                          HEIGHT//2 + i*(MENU_FONT_SIZE + MENU_ITEM_PADDING)
                          <= mouse_pos[1] <=
                          HEIGHT//2 + i*(MENU_FONT_SIZE + MENU_ITEM_PADDING) +
                          MENU_FONT_SIZE)
                         else MENU_COLOR)
...
def draw_high_scores():
    ...
    high_score_image = pygame.transform.scale(high_score_image, (WIDTH,
                                                                 HEIGHT))
    ...
    # Draw high score
    font = pygame.font.SysFont(None, MENU_FONT_SIZE)
    score_text = font.render(f"Best: {high_score}", True,
                             MENU_COLOR)
...
def draw_instructions():
    # Load backgorund
    instruction_image = (pygame.image.load("instruction_background.png")
                         .convert_alpha())
    instruction_image = pygame.transform.scale(instruction_image,
                                               (WIDTH, HEIGHT))
    ...
    # Draw title
    title_font = pygame.font.SysFont(None, MENU_TITLE_SIZE)
    title_text = title_font.render("TIPS & INSTRUCTIONS", True,
                                   MENU_TITLE_COLOR)
    ...
    # Draw instructions
    font = pygame.font.SysFont(None, MENU_FONT_SIZE - 10)  # Slightly
                                                                # smaller font
    ...
    for i, line in enumerate(instruction_lines):
        ...
        if not line.strip():
            start_y += line_height // 2  # Add half line height for paragraph
                                            # spacing
            continue
    ...
    back_text = back_font.render("Press ESC to return to menu",
                                 True, MENU_COLOR)
    back_rect = back_text.get_rect(center=(WIDTH//2, HEIGHT - 25))
    screen.blit(back_text, back_rect)

def handle_menu_click(pos):
    global current_state, GAME_ACTIVE, score, background, player, ai_cars, \
        high_score
        ...
        if (WIDTH//2 - 100 <= pos[0] <= WIDTH//2 + 100 and
            ...
            if item["action"] == "quit":
                ...
                if current_state == GAME_ACTIVE:
                    ...
                    player = Player(CAR_WIDTH, CAR_LENGTH)
...
while True:
    for event in pygame.event.get():
        ...
        if current_state == INSTRUCTIONS and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                current_state = MENU
